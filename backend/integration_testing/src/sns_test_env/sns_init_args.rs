...
impl SnsInitArgs {
    ...
    pub fn new(
        ...
        let governance_args = generate_sns_init_args(neuron_data, sns_ledger, sns_root, sns_swap);
    ...
    pub fn with_token(mut self, symbol: &str, fee: u64) -> Self {
        self.ledger_args.token_symbol = symbol.to_string();
        self.ledger_args.transfer_fee = Nat::from(fee);
        self.swap_args.transaction_fee_e8s = Some(fee);
        self
    }
}
