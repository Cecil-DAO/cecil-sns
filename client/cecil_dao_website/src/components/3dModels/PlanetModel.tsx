...
interface NewArcRotateCamera extends ArcRotateCamera {
    spinTo: (whichprop: 'alpha' | 'beta' | 'radius' | 'target', targetval: number, speed: number) => void;
}
...
const PlanetModel = () => {
    ...
    // const lionBtn2 = useRef<any>();
    // const lionBtn3 = useRef<any>();
    ...
    const newArcRotateCamera = ArcRotateCamera.prototype as NewArcRotateCamera;

    newArcRotateCamera.spinTo = function (whichprop, targetval, speed) {
                ...
                cache.match(request).then((response) => {
                    if (response) {
                        ...
                        response.blob().then((data) => {
                            // 创建一个 URL 对象，用于加载从服务器获取的 .glb 文件
        ...
        SceneLoader.Append(
            ...
            (meshes) => {
                ...
                scene.onPointerObservable.add((pointerInfo) => {
                    if (
                        [PointerEventTypes.POINTERUP, PointerEventTypes.POINTERWHEEL].includes(
                            pointerInfo.type,
                        )
                    ) {
                        if (timeout !== null) {
                ...
                scene.registerBeforeRender(() => {
                    ...
                    const initscale = scale === 1 ? 2 : scale === 2 ? 1 : scale === 3 ? 2 / 3 : 1;
    ...
    return (
        <section className="threeDContent relative h-full w-full text-[#000]">

            <div className="threeDPage">
                {!isMobile && (
                    <div
                        className="backIcon hoverScale absolute left-[20px] top-[80px] z-10 cursor-pointer text-[30px] md:left-[4.5vw] md:top-[100px]"
                    >
                        <img alt="" src="" />
                    </div>
                )}
                {!loading && (
                    <div className="page">
                        ...
                        {/* <div className="btn" ref={lionBtn}>
                            ...
                            <div className="btnText" ref={lionBtn}>
                                Cecil The Lion Project 2
                            </div>
                        </div>
                        ...
                        <div className="btn" ref={lionBtn}>
                            ...
                            <div className="btnText" ref={lionBtn3}>
                                Cecil The Lion Project 3
                            </div>
                        </div> */}
                    </div>
                )}
